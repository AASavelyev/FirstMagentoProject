<?php
/**
 * Oggetto Web extension for Magento
 *
 * NOTICE OF LICENSE
 *
 * This source file is subject to the Open Software License (OSL 3.0)
 * that is bundled with this package in the file LICENSE.txt.
 * It is also available through the world-wide-web at this URL:
 * http://opensource.org/licenses/osl-3.0.php
 *
 * DISCLAIMER
 *
 * Do not edit or add to this file if you wish to upgrade
 * the Oggetto Attribute module to newer versions in the future.
 * If you wish to customize the Oggetto Attribute module for your needs
 * please refer to http://www.magentocommerce.com for more information.
 *
 * @category   Oggetto
 * @package    Oggetto_Attribute
 * @copyright  Copyright (C) 2016 Oggetto Web ltd (http://oggettoweb.com/)
 * @license    http://opensource.org/licenses/osl-3.0.php  Open Software License (OSL 3.0)
 */

/**
 * customer edit block
 *
 * @category   Oggetto
 * @package    Oggetto_Attribute
 * @author     Alexander Savelyev <asavelyev@oggettoweb.com>
 */
class Oggetto_Attribute_Block_Adminhtml_CustomerEdit extends Oggetto_Attribute_Block_Adminhtml_Base_Edit
{
    /**
     * init Edit Customer Block
     */
    public function __construct()
    {
        $this->_mode = 'customerEdit';
        parent::__construct();

        /*$attributeId = (int)$this->getRequest()->getParam('attribute_id');
        $attribute = Mage::getModel('catalog/resource_eav_attribute')->load($attributeId);

        $validateRules = unserialize($attribute->getValidateRules());
        if (isset($validateRules['max_text_length'])) {
            $attribute->setMaxTextLength($validateRules['max_text_length']);
        }
        if (isset($validateRules['min_text_length'])) {
            $attribute->setMinTextLength($validateRules['min_text_length']);
        }

        Mage::register('current_attribute', $attribute);*/
    }
}
